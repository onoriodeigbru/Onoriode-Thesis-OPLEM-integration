...
for i, (ax, market) in enumerate(zip(axes, markets)):
    label = f"{string.ascii_lowercase[i]})"

    v = load(f"Results/{market}/voltage.p")
    if v is None:
        ax.set_title(f"{label} {market} — no data")
        continue
    ...
    # ✅ Automatic lettering in title
    ax.set_title(f"{label} {market}", fontsize=13, fontweight='bold')
...
for col, market in enumerate(markets):
    ...
    ax = axes[0, col]
    label = f"{string.ascii_lowercase[col]})"  # a), b), c)
    ...
    if eb is not None:
        ...
        ax.set_title(f"{label} {market} — Battery SoE",
        ...
        ax.set_title(f"{label} {market} — Battery SoE (no data)")

    # --- Indoor Temperature ---
    ax = axes[1, col]
    label = f"{string.ascii_lowercase[col + len(markets)]})"  # d), e), f)
    ...
    if tin is not None:
        ...
        ax.set_title(f"{label} {market} — Indoor Temperature",
                     fontsize=12, fontweight='bold')
        ...
        ax.set_title(f"{label} {market} — Temperature (no data)")
...
for market in markets:
    ...
    for p_idx, participant_schedules in enumerate(raw):
        ...
        # ✅ Automatic lettering
        label = f"{string.ascii_lowercase[p_idx]})"

        if participant_schedules is None or len(participant_schedules) == 0:
            ax.set_title(f"{label} P{p_idx+1} — no data")
            ax.axis('off')
            continue
        ...
        for a_idx, sched in enumerate(participant_schedules):
            ...
            ax.plot(hours(T), sched,
                    ...
                    lw=1.2,
                    label=f"Asset {a_idx+1}")

        ax.axhline(0, color='black', lw=0.8, ls='--')

        # ✅ Updated title with label
        ax.set_title(f"{label} Participant {p_idx+1}",
                     fontsize=10, fontweight='bold')

        ax.set_xlabel("Time (h)", fontsize=8)
        ax.set_ylabel("Power (kW)", fontsize=8)
        ax.set_xticks([0, 8, 16, 23],
                      ('00:00', '08:00', '16:00', '23:00'))
        ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(f"Plots/schedules_{market}.png",
