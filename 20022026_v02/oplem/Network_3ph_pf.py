...
#Changes made by Onoriode Syverous Igru to fix the issue of file path handling 
import pathlib 
#--------- added 22032026
import pandas as pd
import numpy as np
...
class Network_3ph:
    ...
    def linear_model_setup(self, v_net_lin0, S_wye_lin0, S_del_lin0):
        ...
        del_mat = np.array([[1,-1,0],
                            [0,1,-1],
                            [-1,0,1]],dtype=np.complex128)#a-b,b-c,c-a
        ...
        v_lin0_diag_inv = np.zeros([v_lin0.shape[0],
                                    v_lin0.shape[0]],dtype=np.complex128)
    ...
    def linear_pf(self):
        ...
        """
        S_loads = np.zeros([self.Y.shape[0]],dtype=np.complex128) 
            #S loads
        S_PQ_wye = np.zeros([self.Y.shape[0]],dtype=np.complex128) 
            #S of PQ loads (wye)
        S_PQ_del = np.zeros([self.Y.shape[0]],dtype=np.complex128) 
            #S of PQ loads (delta)
        for bus_i in range(1,len(self.bus_df)):
...
#                                   + self.bus_df.iloc[bus_i]['Qc']*1j)*1e3
        ...
        self.vs = np.array([1,aaa**2,aaa],dtype=np.complex128)*self.Vslack_ph 
        #slack node voltage. 
        v = np.zeros([self.N_iter,self.Y.shape[0]],dtype=np.complex128)
        ...
        i_PQ = np.zeros([self.N_iter,self.Y.shape[0]],dtype=np.complex128)
        #del_mat = np.array([[1,-1,0],[0,1,-1],[-1,0,1]],dtype=np.complex128)
        #del_blkmat = block_diag(*([del_mat]*(len(self.bus_df)-1)))
    ...
    def update_YandZ(self):
        ...
        for line_ij in range(self.N_lines):
            ...
            Yseries = np.zeros([3,3],dtype=np.complex128)
        ...
        for cap_index in range(len(self.capacitor_df)):
            ...
            Ymat = np.zeros([3,3],dtype=np.complex128)
            for phase_i in range(3):
                for phase_j in range(3):
                    if phases[phase_i] and phases[phase_j]:
    ...
    def setup_network_eulv_reduced(self, updateYZ=True):
        ...
        path = importlib.resources.files('data')
        #issue
        #file_location = os.path.join(str(path),  'networks', 'eulv_reduced/')
        #replacement
        file_location = r"D:\Work\Stanley_Friend_(19OCT2025)\20022026_v02\data\networks\eulv_reduced"
        buses_csv = pd.read_csv(os.path.join(file_location , 'eulv_bus_df.csv'))
        lines_csv = pd.read_csv(os.path.join(file_location , 'Lines2.txt'), sep=' ', names=["new", "name", "busA", "busB", "phases", "linecode", "length", "units"])
        #
    ...
    def setup_network_ieee123(self, updateYZ=True):
        ...
        path = importlib.resources.files('data')
        #issue
        #file_location = os.path.join(str(path),  'networks', 'feeder123/')
        #replacement
        file_location = pathlib.Path(path) / "networks" / "feeder123/"

        bus_load_data = pd.read_excel(str(file_location / 'spot loads data.xls'), header=[2, 3])
    ...
    def loadDssNetwork(self,ntwkName,updateYZ=True,
        ...
        #sys.path.insert(0, os.path.join(str(path),'System'))
        #os.path.dirname(os.path.dirname(path))
        #path = os.path.dirname(os.path.abspath(__file__))
        #path = os.path.dirname(os.path.dirname(__file__))
        path = importlib.resources.files('data')
        self.feederLoc = os.path.join(str(path),'networks', ntwkName+'_dss',ntwkName)
        ...
        for i in range(2,line_df.shape[1]):
            for j in range(line_df.shape[0]):
                line_df.iloc[j,i] = np.complex128(np.complex(line_df.iloc[j,i])) 
        ...
        if os.path.exists(self.feederLoc+"_trn_df.csv"):
            ...
            for i in [4,5]: # convert to (exactly) same format
                for j in range(trn_df.shape[0]):
                    trnNo = np.complex128(np.complex(trn_df.iloc[j,i])) 
                    if i==5 and trnNo==0:
