...
class CED_market(Market):
	...
	def market_clearing(self, v_unconstrained_buses=[], i_unconstrained_lines=[]):
		...
		"""
		#v_unconstrained_buses = list(range(500))

		def ensure_array(name, arr, ref):
			if arr is None:
				print(f"⚠️ {name} missing → using zeros")
				return np.zeros_like(ref)
			arr = np.asarray(arr, dtype=float)
			arr = np.nan_to_num(arr, nan=0.0, posinf=1e6, neginf=-1e6)
			return arr

		self.P_import = ensure_array("P_import", self.P_import, np.zeros(24))
		self.P_export = ensure_array("P_export", self.P_export, self.P_import)

		if not hasattr(self, "prices"):
			self.prices = None

		self.prices = ensure_array("prices", self.prices, self.P_import)
		...
		if self.nw_const:	
			...
			for t_ems in range(self.T_market-self.t_ahead_0):
				...
				"""prob.add_list_of_constraints([pbus[t_ems, bus] ==
						# ND assets (fixed loads + PV) — was missing before
						sum(P_demand[t_ems, k]
							for k in index_assets_nd_per_bus[bus])
						# curtailable load assets
						+ sum(P_demand[t_ems, len(participant_all.assets_nd)+k]-p_curt[t_ems, k]
							for k in index_assets_load_per_bus[bus])
						# curtailable gen assets
						+ sum(P_demand[t_ems, len(participant_all.assets_nd)+k]-p_curt[t_ems, k]
							for k in index_assets_gen_per_bus[bus])
						# flexible assets
						+ sum(x[t_ems, i] + x[self.T_market-self.t_ahead_0+t_ems, i]
							for i in index_assets_flex_per_bus[bus])
					for bus in range(len(buses))])"""
				#### end lines for dlmps
			...
			for t_ems in range(self.T_market-self.t_ahead_0):	
				...
				# voltage magnitude constraints with tolerance:
				v_tol = 0
			
				for bus_ph_index in range(self.network.N_phases*(self.network.N_buses-1)):
					if int(bus_ph_index/3) not in v_unconstrained_buses: 
						prob.add_constraint(sum(A_vlim[bus_ph_index,b]*1e6*pbus[t_ems, b] for b in range(len(buses)))
											+ b_vlim[bus_ph_index] <= v_abs_max_vec[bus_ph_index] + v_tol)
						prob.add_constraint(sum(A_vlim[bus_ph_index,b]*1e6*pbus[t_ems, b] for b in range(len(buses)))
											+ b_vlim[bus_ph_index] >= v_abs_min_vec[bus_ph_index] - v_tol)
		...
		try:
			...
			# Add bounds and retry
			print("\nAdding bounds and retrying...")
			prob.add_constraint(Pimp <= 1e6)
			prob.add_constraint(Pexp <= 1e6)
			prob.add_constraint(pbus >= -1e6)
			prob.add_constraint(pbus <= 1e6)
			try:
				prob.solve(solver='mosek')
				print('Status after bounding:', prob.status, prob.value)
			except Exception as e2:
				print(f'Still failed after bounding: {e2}')
				print(f'Status: {prob.status}')	
		...
		if len(participant_all.assets_flex):
			...
			if x_val.ndim == 0:
				x_val = np.zeros((2*(self.T_market-self.t_ahead_0), len(participant_all.assets_flex)))
			x = x_val
		...
		if len(participant_all.assets_curt):
			...
			if p_curt_val.ndim == 0:
				p_curt_val = np.zeros((self.T_market-self.t_ahead_0, len(participant_all.assets_curt)))
			p_curt = p_curt_val
		...
		for p_idx, par in enumerate(self.participants):
			...
			for asset in par.assets:
				...
				if asset.type == 'curt':			
					...
					asset.update_ems(x[:self.T_market-self.t_ahead_0, flex]+ x[self.T_market-self.t_ahead_0:, flex], 
					  self.t_ahead_0, enforce_const=False)
					"""x_sol = x

					asset.update_ems(
						x_sol[:self.T_market-self.t_ahead_0, flex]
						+ x_sol[self.T_market-self.t_ahead_0:, flex],
						self.t_ahead_0,
						enforce_const=False
					)"""
				...
				for t in range(self.T_market-self.t_ahead_0):
					if self.nw_const:
						price_t = float(DLMP[t, bus])
					else:
						price_t = float(self.price_imp[t+self.t_ahead_0])

					if sch[t] > 0:
						list_clearing.append([
							t+self.t_ahead_0,
							0,              # seller = grid
							par.p_id,       # buyer = participant
							self.dt_market*sch[t],
							price_t
						])
					elif sch[t] < 0:
						if not self.nw_const:
							price_t = float(self.price_exp[t+self.t_ahead_0])
						list_clearing.append([
							t+self.t_ahead_0,
							par.p_id,       # seller = participant
							0,              # buyer = grid
							-self.dt_market*sch[t],
							price_t
						])
...
class ToU_market(Market):
	...
	def __init__(self, participants, dt_market, T_market, price_imp,\
		...
		"""self.P_import = np.array("P_import", self.P_import, np.zeros(24))
		self.P_export = np.array("P_export", self.P_export, self.P_import)

		if not hasattr(self, "prices"):
			self.prices = None

		self.prices = np.array("prices", self.prices, self.P_import)"""
		Market.__init__(self, participants, dt_market, T_market, price_imp, t_ahead_0=t_ahead_0, P_import=P_import, P_export=P_export, price_exp=price_exp, network=network)
...
class P2P_market(Market):
	...
	def _get_trades(self,participant, trade_list, q_trades_req=None):  
		...
		if len(participant.assets_flex):
			prob.set_objective('min',    pic.sum(self.price_imp[t+self.t_ahead_0, participant.p_id-1]*q_sup_buy[t] 
				                          - self.price_exp[t+self.t_ahead_0, participant.p_id-1]*q_sup_sell[t]  for t in range(T_dec)
				                            )\
				                           # 
			                            + pic.sum((prices_us[trd]+ trade_tcost_us[trd]/2)*q_us[trd]  for trd in range(N_trades_us))\
			                            + pic.sum((-prices_ds[trd]+ trade_tcost_ds[trd]/2)*q_ds[trd]  for trd in range(N_trades_ds))\
