...
#print("P_import min/max:", np.min(self.P_import), np.max(self.P_import))
#print("Load sample:", Loads_actual[:5])
#print("PV sample:", PVpu[:5])
...
###1)initialise market
Central = CED_market(participants, dt_ems, T_ems, prices_wsm_ems[:,0], network=network, t_ahead_0 = 0, price_exp=FiT[:,0])
Central.P_import = np.full(T_ems, 1e6)
Central.P_export = np.full(T_ems, 1e6)
...
assert_valid("P_import", Central.P_import)

Central.prices = prices_wsm_ems[:, 0]
Central.loads = Load_ems[:, 0]
